...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CONTEXTMENU:
	{
		...
		switch (TrackPopupMenuEx(hMenu, TPM_BOTTOMALIGN | TPM_LEFTALIGN | TPM_RETURNCMD, LOWORD(lParam)/*координата по х*/, HIWORD(lParam)/*координата по у*/, hwnd, NULL))
		{
...
VOID SetSkin(HWND hwnd, CONST CHAR sz_skin[])
{
	...
	for (int i = 0; i < SIZE; i++)
	{
		...
		sprintf(sz_filename, "ButtonsBMP\\%s\\button_%i.bmp",sz_skin, i);//получили имя файла и записали в файл
	...
	for (int i = 0; i < sizeof(BUTTON_NAMES)/sizeof(BUTTON_NAMES[0]); i++)
	{
		...
		sprintf(sz_filename, "ButtonsBMP\\%s\\button_%s.bmp", sz_skin, BUTTON_NAMES[i]);//получили имя файла и записали в файл
		HBITMAP hBitmap = (HBITMAP)LoadImage
		(
			GetModuleHandle(NULL),
			sz_filename,
